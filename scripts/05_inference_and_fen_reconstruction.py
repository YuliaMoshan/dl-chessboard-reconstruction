#!/usr/bin/env python3
"""Run inference and reconstruct board FENs from square predictions.

Loads a trained classifier, predicts per-square labels, applies a rule-based
repair step, renders board images, and writes inference metrics/debug CSVs.
"""
...
THRESHOLD = 0.75
# Predictions below this confidence become "X" (unknown / OOD).
...
def main():
    ...
    board_exact_match = 0
    board_total = 0
    board_exact_match_no_occlusion = 0
    board_total_no_occlusion = 0
    ...
    with torch.no_grad():
        for (game_id, frame_id), g in tqdm(grouped, desc="Reconstructing boards"):
            ...
            is_occluded_val = g["is_occluded"].iloc[0]
            if isinstance(is_occluded_val, str):
                is_occluded = is_occluded_val.strip().lower() == "true"
            else:
                is_occluded = bool(is_occluded_val)
            ...
            if not is_occluded:
                if board_correct:
                    board_exact_match_no_occlusion += 1
                board_total_no_occlusion += 1
    ...
    print(f"Full board match rate:  {board_exact_match / board_total:.4f}")
    if board_total_no_occlusion:
        print(f"Full board match rate (non-occlusion):  {board_exact_match_no_occlusion / board_total_no_occlusion:.4f}")
        print(f"Total non-occluded boards evaluated:     {board_total_no_occlusion}")
